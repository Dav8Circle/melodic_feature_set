...
def main():
    ...
    if args.format == "csv":
        ...
        with open(args.out, "w", encoding="utf-8") as f:
            ...
            f.write(f"_qmd_path = Path({out_path.as_posix()!r})\n")
            f.write("_qmd_dir = _qmd_path.parent\n")
            f.write("repo_root = _qmd_dir if (_qmd_dir / 'src' / 'melody_features').exists() else _qmd_dir.parent\n")
            f.write("if not (repo_root / 'src' / 'melody_features').exists():\n")
            f.write("    repo_root = Path.cwd()\n")
            f.write("src_dir = repo_root / 'src'\n")
            f.write("docs_dir = repo_root / 'docs'\n")
